...
from Custom_Layers.SCGP           import SelfCorrectingBlock
...
class ModifiedResNet18Block(nn.Module):
	def __init__(self, in_channels, out_channels, stride=1, use_adabn=False, use_cbam=False, use_proto=False, use_rbn=False, use_TC=False, use_scb=False):
		super().__init__()
		self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False)
...
class ResNet18(nn.Module):
	...
	def _make_layer(self, in_channels, out_channels, blocks, stride):
		layers = []
		layers.append(ModifiedResNet18Block(in_channels, out_channels, stride, self.use_adabn, self.use_cbam, self.use_proto, self.use_rbn, self.use_scb))
		for _ in range(1, blocks):
			layers.append(ModifiedResNet18Block(out_channels, out_channels, 1, self.use_adabn, self.use_cbam, self.use_proto, self.use_rbn, self.use_scb))
